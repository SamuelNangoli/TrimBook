...
import {
  ...
  type BookState,
  type SlotDTO,
} from "@/server/actions/public-booking.actions";
...
export function PublicBookingForm({
  ...
  useEffect(() => {
    ...
    setSelected("");
    startLoading(async () => {
      setSlots(await getPublicSlotsAction({ slug, serviceId, barberId, dateStr }));
    });
  }, [slug, serviceId, barberId, dateStr]);
  ...
  return (
    <form action={action} className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        {loading ? (
          ...
          <p className="text-sm text-muted-foreground">
            No open slots for this day. Try another date or barber.
          </p>
        ) : (
      ...
      <Button type="submit" disabled={submitting || !selected} className="min-h-11 w-full">
        {submitting ? "Booking…" : "Confirm booking"}
      </Button>
      {!selected && slots.length > 0 && (
        <p className="text-center text-xs text-muted-foreground">Pick a time slot to continue.</p>
      )}
    </form>
  );
}
